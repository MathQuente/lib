...
async function main() {
  ...
  while (true) {
    ...
    const infoMap = await IGDBService.getCategoryAndParentByIds(igdbIds)
    await repo.updateCategoryAndParent(
      igdbIds.map(igdbId => ({
        igdbId,
        category: infoMap.get(igdbId)?.category ?? -1,
        parentGameId: infoMap.get(igdbId)?.parentGameId ?? null
      }))
    )
