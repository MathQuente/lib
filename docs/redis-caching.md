...
- Convenção de chave por tipo de dado, usando os parâmetros que já afetam cada resultado:
  ...
  - "trending" e "mais avaliados": chave incluindo o `limit` pedido, com prefixo próprio para cada um dos dois.
...
- TTL diferente por volatilidade real do dado:
  - dados que vêm só da IGDB e mudam raramente (jogo por id, similares, em breve): TTL mais longo, da ordem de dezenas de minutos a poucas horas.
  - "trending" e "mais avaliados" dependem de atividade interna do site (jogos jogados recentemente, médias de avaliação), recalculada com frequência no Postgres — por isso pedem um TTL bem mais curto, de poucos minutos.
- Sorted sets para ranking é uma otimização a avaliar depois, não um pré-requisito: o candidato a sorted set é só o *ranking de ids* (hoje resultado de uma agregação no Postgres), não os dados completos do jogo — esses continuam usando o mesmo cache por-id do item anterior. Vale considerar essa estrutura se o recálculo da agregação no Postgres se mostrar de fato um gargalo; começar cacheando só a chamada à IGDB já resolve a maior parte do problema.
